...
<ul> <li><h3>$fname $lname</h3></li> 
   ...
   <li><a  href='messages.php?to=$user' >Message</a></li>
    </ul>
";}
?>
</div>
...
<?php
include("db/config.php");
$data="select * from posts where username='$_GET[rec]' order by id desc" ;
$run=mysqli_query($conn,$data);
...
<?php
include("db/config.php");
$data="select day,month,year,email,gen from users  where uname='$_GET[rec]' " ;
$run=mysqli_query($conn,$data);
...
$data="select pic,date from posts where username='$_GET[rec]' order by id desc" ;
$run=mysqli_query($conn,$data);
if($run){
